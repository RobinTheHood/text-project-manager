...
class EvaluationCreator extends AbstractCreator
{
    ...
    private function renderTaskEvaluationDTO(TaskEvaluationDTO $taskEvaluationDTO): string
    {
        ...
        /** @var DurationReportCondensate */
        foreach ($actual['condensates'] as $condensate) {
            $actualHours = $this->formatHours($condensate->getHours());
            $internalPrice = $this->formatCurrency($condensate->getInternalPrice());
            $totalInternalPrice = $this->formatCurrency($condensate->getTotalInternalPrice());
        ...
        if ($task->target && $task->target->value) {
            ...
            foreach ($target['condensates'] as $condensate) {
                $targetHoursRounded = $this->formatHours($condensate->getHoursRounded());
                $externalPrice = $this->formatCurrency($condensate->getTotalExternalPrice());
                $totalExternalPriceRounded = $this->formatCurrency($condensate->getTotalExternalPriceRounded());
    ...
    private function calculateActualFromReports(array $reports): array
    {
        ...
        foreach ($condensates as $condensate) {
            $actualPrice += $condensate->getInternalPrice();
        }
    ...
    private function calculateTargetFromReports(array $reports): array
    {
        ...
        /** @var DurationReportCondensate */
        foreach ($condensates as $condensate) {
            $targetPrice += $condensate->getTotalExternalPriceRounded();
        }
